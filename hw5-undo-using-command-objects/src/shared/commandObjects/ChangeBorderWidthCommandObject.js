...
export default class ChangeBorderWidthCommandObject extends CommandObject {
  ...
  execute() {
    if (window.selectedObj) {
      this.type = 'ChangeBorderWidth';
      this.targetObject = window.selectedObj; // global variable for selected
      this.oldValue = window.selectedObj.borderWidth; // object's current color
      this.newValue = borderWidget.currentWidth; // get the color widget's current color
      window.selectedObj.borderWidth = this.newValue; // actually change
  ...
  undo() {
    ...
    window.selectedObj.borderWidth = this.oldValue;
  ...
  redo() {
    window.selectedObj = this.targetObject;
    window.selectedObj.borderWidth = this.newValue;
    // maybe also need to fix the palette to show this object's color?
  }
